...
from core.models import PNet, LossFn, compute_accuracy


def parse_args():
    parser = argparse.ArgumentParser(description='Train PNet')
    parser.add_argument('--end_epoch', dest='end_epoch', help='end epoch of training',
                        default=config.PNET_end_epoch, type=int)
...
def train_pnet(model_store_path, annotation_file, end_epoch, frequent=200, gpu=None, restore=False, restore_model_path=None):
    ...
    if gpu:
        ...
        if restore:
            ...
            weights.pop("conv4_3.weight")
            weights.pop("conv4_3.bias")
            net_dict.update(weights)
            net.load_state_dict(net_dict)
        net = torch.nn.DataParallel(net).cuda()
    else:
        net = PNet(is_train=True, use_cuda=False)
        if restore:
            net.load_state_dict(torch.load(restore_model_path))
    net = net.train()
    optimizer = torch.optim.Adam(net.parameters(), lr=config.PNET_lr)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=config.PNET_step_size, gamma=config.PNET_gamma)
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=config.PNET_batch_size, shuffle=True, num_workers=8, pin_memory=True, drop_last=True)
    ...
    for epoch in range(1, end_epoch + 1):
        ...
        for iteration, (image, gt_label, gt_bbox, gt_landmark) in enumerate(train_loader):
            ...
            if iteration % frequent == 0:
                ...
                print("Epoch: %d, Step: %d, accuracy: %s, det loss: %s, bbox loss: %s, all_loss: %s"
                      % (epoch, iteration, show1, show2, show3, show5))
            """
        print("Epoch: %d, all loss: %f, cls loss: %f, box_loss: %f"
              % (epoch, total_all_loss/index, total_cls_loss/index, total_box_loss/index))

        #random_seed = np.random.randint(10000, 99999)
        torch.save(net.state_dict(), os.path.join(model_store_path, "pnet_epoch_%d.pth" % (epoch)))
        torch.save(net, os.path.join(model_store_path, "pnet_epoch_model_%d.pkl" % (epoch)))
...
if __name__ == '__main__':
    ...
    train_pnet(config.PNET_model_store_path, config.PNET_TRAIN_IMGLIST_PATH, args.end_epoch, args.frequent,\
