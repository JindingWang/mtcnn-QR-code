...
from core.models import ONet, LossFn


def parse_args():
    parser = argparse.ArgumentParser(description='Train ONet')
    parser.add_argument('--end_epoch', dest='end_epoch', help='end epoch of training',
                        default=config.ONET_end_epoch, type=int)
    ...
    parser.add_argument('--restore', dest='restore', help='retore model',
                        default=config.ONET_restore, type=bool)
    parser.add_argument('--restore_model_path', dest='restore_model_path', help='retore model\'s path',
                        default=config.ONET_restore_model_path, type=str)
    args = parser.parse_args()
    return args


def train_onet(model_store_path, annotation_file, end_epoch, frequent=200, gpu=None, restore=False, restore_model_path=None):
    if not os.path.exists(model_store_path):
        os.makedirs(model_store_path)

    lossfn = LossFn()
    if gpu:
        net = ONet(is_train=True, use_cuda=True)
        if restore:
            net_dict = net.state_dict()
            weights = torch.load(restore_model_path)
            weights.pop("conv6_3.weight")
            weights.pop("conv6_3.bias")
            net_dict.update(weights)
            net.load_state_dict(net_dict)
        net = torch.nn.DataParallel(net).cuda()
    else:
        net = ONet(is_train=True, use_cuda=False)
        if restore:
            net.load_state_dict(torch.load(restore_model_path))
    net = net.train()
    optimizer = torch.optim.Adam(net.parameters(), lr=config.ONET_lr)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=config.ONET_step_size, gamma=config.ONET_gamma)
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        batch_size=config.ONET_batch_size, shuffle=True, num_workers=8, pin_memory=True, drop_last=True)
    ...
    for epoch in range(1, end_epoch + 1):
        ...
        for iteration, (image, gt_label, gt_bbox, gt_landmark) in enumerate(train_loader):
            ...
            all_loss = cls_loss * 1.0 + box_offset_loss * 0.5 + landmark_loss * 1.0
        ...
        torch.save(net.state_dict(), os.path.join(model_store_path, "onet_epoch_%d.pth" % (epoch)))
        torch.save(net, os.path.join(model_store_path, "onet_epoch_model_%d.pkl" % (epoch)))
...
if __name__ == '__main__':
    ...
    train_onet(config.ONET_model_store_path, config.ONET_TRAIN_IMGLIST_PATH, args.end_epoch, args.frequent,\
