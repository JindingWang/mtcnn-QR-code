...
save_path = '/home/wjd/datasets/QRcode/Otrain'
pnet_model_file = '../save_model/pnet_epoch_20.pth'
rnet_model_file = '../save_model/rnet_epoch_20.pth'
annotation_file = os.path.join("../annotation", 'anno_train.txt')


def gen_onet_data(data_dir, anno_file, pnet_model_file, rnet_model_file, gpu='', vis=False):
    pnet, rnet, _ = create_mtcnn_net(p_model_path=pnet_model_file, r_model_path=rnet_model_file, gpu=gpu)
    mtcnn_detector = MtcnnDetector(pnet=pnet, rnet=rnet, min_face_size=24)
    ...
    for index in range(num):
        ...
        p_boxes, p_boxes_align = mtcnn_detector.detect_pnet(im=image)
        boxes, boxes_align = mtcnn_detector.detect_rnet(im=image, dets=p_boxes_align)
...
def gen_onet_sample_data(data_dir, anno_file, det_boxs_file):
    ...
    gen_aug_data(data_dir, anno_file)
...
if __name__ == '__main__':
    ...
    #gen_onet_data(save_path, annotation_file, pnet_model_file, rnet_model_file, gpu=args.gpu)

    #gen_aug_data(save_path, annotation_file)

    f1 = open(os.path.join("../annotation", 'pos_%d.txt' % 48), 'r')
    f2 = open(os.path.join("../annotation", 'neg_%d.txt' % 48), 'r')
    f3 = open(os.path.join("../annotation", 'part_%d.txt' % 48), 'r')
    f4 = open(os.path.join("../annotation", "landmark_48.txt"), 'r')
    pos = f1.readlines()
    neg = f2.readlines()
    part = f3.readlines()
    aug = f4.readlines()

    if os.path.exists(os.path.join("../annotation", "Onet_anno_48.txt")):
        os.remove(os.path.join("../annotation", "Onet_anno_48.txt"))

    f5 = open(os.path.join("../annotation", "Onet_anno_48.txt"), 'w')

    nums = [len(neg), len(pos), len(part)]
    ratio = [3, 1, 1]
    # base_num = 250000
    print(len(neg), len(pos), len(part), len(aug))
    ...
    pos_keep = np.random.choice(len(pos), size=len(pos), replace=True)
    part_keep = np.random.choice(len(part), size=len(part), replace=True)
